...
class handler(tornado.web.RequestHandler):
    def get(self):
        ...
        for i in range(0,UserCartListLen):
            ...
            UserCartItems += "<div class=\"CIt\" id=\"CIt_{0:d}\" prc=\"{8:f.2}\" style=\"top:{1:d}px;\"><input class=\"CIq\" type=\"number\" value=\"{2:d}\" onblur=\"AdjOa()\"{3:s}><a href=\"/product/{0:d}/\" class=\"CIi\" style=\"background-image:url({4:s});\"></a>{5:s}<h3>{6:s}</h3><h1>{7:s}</h1>{9:s}</div>\n".format(UserCartItem_ID,(i*210),int(UserCartList[i][1]),UserCartAdjustNumDisable,UserCartItem_ImgLink,UserCartItem_TemplateSet,FindProduct(UserCartItem_ID)["Name"],UserCartItem_PriceSet,UserCartItem_Price,UserCartRemoveButton)
        if UserCartItems != "":
            if UserCurrencySymbol in SpecifyCurrencyList:
            ...
            UserBillingAddress = "<p>0001 Street Name Rd Apt #1000</p><p>New York, NY 00000-0000</p><p>United States</p>"
