...
class handler(tornado.web.RequestHandler):
    def get(self):
        ...
        if UserCartItems != "":
            if UserCurrencySymbol in SpecifyCurrencyList:
                UserCartTotalsSet = "<h3>Shipping: {0:s}<u id=\"CICTt\">{1:,.2f}</u> ({2:s})</h3><h3 id=\"CICt\">Total: {0:s}<u id=\"CICTt\">{3:,.2f}</u> ({2:s})</h3>".format(UserCurrencySymbol,UserCartListShipping,UserCurrency,UserCartListTotal)
            else:
